...
public class WindowManager {
	...
	protected static void updateMenu() {
		synchronized (windows) {
			...
			for (int i=0; i<windows.size(); i++) {
				...
				finalMenu.addElement(new MysterMenuItemFactory(frame.getTitle(), new OtherWindowHandler(frame)));
			}
	...
	private static class CycleWindowsHandler implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			...
			synchronized (windows) {
				if (windows.size()<=0) return; //(could happen if none-tracked window is frontmost)
