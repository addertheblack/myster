...
public class PongTransport extends DatagramTransport {
	...
	*/
	...
	public void ping(MysterAddress param_address, PingEventListener listener) throws IOException { //DANGER DEADLOCKS!
		synchronized (requests) {
	...
	private class PongItemStruct {
		...
		public PongItemStruct(MysterAddress param_address) {
			...
			timer=new Timer(new TimeoutClass(param_address), FIRST_TIMEOUT+(1*1000), false);
		}
	}
				...
				if (struct!=null) {
					if (!struct.secondPing) {
						...
						if (struct.timeStamp>(curTime-TIMEOUT)) {
							//System.out.println("Assumption bad in Pong transport");
							//System.exit(1);
						}
					}
				} else {
