...
public class StandardSuite {
	...
	public static Vector getSearch(MysterAddress ip, MysterType searchType,
	...
	//Vector of strings
	public static Vector getSearch(MysterSocket socket, MysterType searchType,
			String searchString) throws IOException {
		...
		for (String temp = socket.in.readUTF(); !temp.equals(""); temp = socket.in
				.readUTF())
	...
	public static Vector getTopServers(MysterAddress ip, MysterType searchType)
			throws IOException {
		MysterSocket socket = null;
		try {
			socket = MysterSocketFactory.makeStreamConnection(ip);
	...
	public static Vector getTopServers(MysterSocket socket,
			MysterType searchType) throws IOException {
	...
	private static MysterType[] getTypesVersion1Protocol(MysterSocket socket)
			throws IOException {
		...
		for (String temp = socket.in.readUTF(); !temp.equals(""); temp = socket.in
			...
			try {
				...
				throw new ProtocolException(
						"Server sent a malformed MysterType");
			}
		}
	...
	public static RobustMML getServerStats(MysterSocket socket)
			throws IOException {
	 ...
	 */
	public static void downloadFile(final MysterAddress ip,
	...
	private static class DownloadThread extends com.myster.util.MysterThread {
		...
		public void run() {
			final com.myster.util.FileProgressWindow progress = new com.myster.util.FileProgressWindow(
					"Connecting..");
			...
			try {
				...
				com.general.util.AnswerDialog
						.simpleAlert("Could not connect to server.");
				return;
			}

			try {
				downloadFile(socket, stub, progress);
		...
		private void downloadFile(final MysterSocket socket,
				final MysterFileStub stub, final FileProgressWindow progress)
				throws IOException {
			...
			try {
				...
				if (DONT_USE_MULTISOURCE)
					throw new IOException(
							"Toss and catch: Multisource download diabled");
				...
				final File theFile = MultiSourceUtilities.getFileToDownloadTo(
						stub, progress);
				...
				synchronized (StandardSuite.DownloadThread.this) {
					...
					final MSPartialFile partialFile = MSPartialFile.create(stub
							.getName(), stub.getType(),
							MultiSourceDownload.DEFAULT_CHUNK_SIZE,
							new FileHash[] { hash }, MultiSourceUtilities
									.getLengthFromStats(mml));

					msDownload = new MultiSourceDownload(stub, hash,
							MultiSourceUtilities.getLengthFromStats(mml),
							new MSDownloadHandler(progress, theFile,
									partialFile), new RandomAccessFile(theFile,
									"rw"), partialFile);
				}
				...
				try {
					...
					synchronized (StandardSuite.DownloadThread.this) {
						...
						secondDownload = new DownloaderThread(
								MysterSocketFactory.makeStreamConnection(stub
										.getMysterAddress()), stub, progress);
					}
					secondDownload.run();
				} catch (IOException exp) {
					progress
							.setText("Could not download file by either method...");
				}
			}
		}
	...
	public static RobustMML getFileStats(MysterAddress ip, MysterFileStub stub)
			throws IOException {
		MysterSocket socket = null;
		try {
			socket = MysterSocketFactory.makeStreamConnection(ip);
	...
	public static RobustMML getFileStats(MysterSocket socket,
			MysterFileStub stub) throws IOException {
	...
	public static String getFileFromHash(MysterAddress ip, MysterType type,
			FileHash hash) throws IOException {
	...
	public static String getFileFromHash(MysterSocket socket, MysterType type,
			FileHash hash) throws IOException {
	...
	public static void checkProtocol(DataInputStream in) throws IOException,
		...
		if (err != 1) {
			throw new UnknownProtocolException(err,
