...
public class MSPartialFile {
	...
	public static MSPartialFile create(String filename, MysterType type, int blockSize, FileHash[] hashes, long fileLength) throws IOException {
		File fileReference = new File(MultiSourceUtilities.getIncomingDirectory(), filename + FILE_ENDING);
		
		if (fileReference.exists()) {
			if (! fileReference.delete()) throw new IOException("Cannot create a partial downloa dfile, there's a file in the way.");
		}
		
		RandomAccessFile maskFile 	= new RandomAccessFile(fileReference,"rw");
		PartialFileHeader header 	= new PartialFileHeader(filename, type, blockSize, hashes, fileLength);
	...
	public long getFirstUndownloadedBlock() throws IOException {
		...
		int numberOfBlocks = (int)((maskFile.length()-offset) / buffer.length); //DANGER! UNSAFE CAST TO INT! THIS CODE CAN FAIL FOR LARGE FILE SIZES! (very, very large but whatever)
		
		for (long blockCounter = 0; blockCounter <= numberOfBlocks; blockCounter ++) {
			int currentBlockSize = (int)(blockCounter >= numberOfBlocks ? (maskFile.length()-offset) % buffer.length : buffer.length); //UNSAFE CAST
			...
			for (int i = 0 ; i < currentBlockSize; i++) {
				if (buffer[i]!=((byte)0xFF)) return (8*(i + (blockCounter * blockSize)));
			}
		}
	...
	public void setBit(long bit) throws IOException {
		...
		if (myData == -1) myData = 0; // end of file yeah yeah yeah...
	...
	private static class PartialFileHeader { 
		...
		PartialFileHeader(String filename, MysterType type, long blockSize, FileHash[] hashes, long fileLength) {
		...
		*/
		...
		public com.myster.mml.MML toMML() {
			...
			mml.put(TYPE, ""+type.getAsInt()); //! is encoded as an int instead of a string because the string encoding is not exactly equivalent
