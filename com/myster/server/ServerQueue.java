...
public class ServerQueue extends AbstractDownloadQueue {
	...
	private static final void setQueueLength(String UNIQUE_PATH_IN_PREFS, int i) { //sets the queue length in the !prefs!.
		Preferences.getInstance().put(PREF_PATH+"/"+UNIQUE_PATH_IN_PREFS, ""+i);
	}
	
	private static final int getQueueLength(String UNIQUE_PATH_IN_PREFS, int defaultValue) {	//gets the queue length from the !prefs!.
		String s_spots = Preferences.getInstance().get(PREF_PATH+"/"+UNIQUE_PATH_IN_PREFS);
