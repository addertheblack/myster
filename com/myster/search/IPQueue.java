...
public class IPQueue {
	...
	public synchronized MysterAddress getNextIP() {
		if (head.next!=null) {
			//note usually we'd do head.next.value since this Queue is implement with
			//a "head" based design as in C++ coding. But here we access it as we
	...
	public boolean checkIP(MysterAddress ip) {
		...
		while (index.next!=null) {
			if (index.next.value.equals(ip)) return true;
			index=index.next;			
		}
