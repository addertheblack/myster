...
/**
*	MysteriP objects are responsible for two things. 1) Saving server statistics information and 2) keeping this 
*	information upt to date.
*/
...
class MysterIP {	
	...
	/**
	*	This private class implements the com.myster interface and allows outside objects to get vital server statistics.
	*/
	...
	private class MysterIPInterfaceClass implements MysterServer {
		...
		*/
		...
		public String getServerIdentity() {
			return (serverIdentity==null?""+ip:(serverIdentity.length()>31?serverIdentity.substring(0,31):serverIdentity));
		}	
	...
	/**
	 * The ever famous toString() method!!!
	 * You can't go from string to new Myster IP easily!!!
	 */
	 
	public String toString() {
		toUpdateOrNotToUpdate();
		
		return ""+toMML();		//return the toString() the MML of this MysterIP! Fun eh?
	}
	...
	protected MML toMML() {
		try {
			...
			if(!s_temp.equals("")) workingmml.put(NUMBEROFFILES, 	numberOfFiles.toString());
	...
	*	which means that it won't stop your program from excecuting while it looks up the
	...
	*/
	...
	private synchronized void toUpdateOrNotToUpdate() {
		...
		if ((System.currentTimeMillis()-lastminiupdate<(getMysterCount()>0?MINIUPDATETIME:UPDATETIME))&&(System.currentTimeMillis()-timeoflastupdate<UPDATETIME)) {
			return;
		}
		
		occupied=true; //note, there is an update in progress on this MysterIP
	...
	*/
	private static boolean internalRefreshAll(MysterIP mysterip) {
		...
		try {
			...
			if (System.currentTimeMillis()-mysterip.timeoflastupdate<UPDATETIME) throw new MassiveProblemException("");
				...
				try {
					if (dirList!=null) {
						for (int i=0; i<dirList.size(); i++) {
							String s_temp=mml.get("/numberOfFiles/"+(String)(dirList.elementAt(i)));
							if (s_temp==null) continue; //<- weird err.
	
							table.put("/"+(String)(dirList.elementAt(i)), s_temp);//<-WARNING could be a number
						}
					}
				} finally {
			...
			//ex.printStackTrace();
		 	...
		 	//System.out.println("Some sort of problem stopped "+mysterip.ip+" from being refreshed.");
		} catch (Exception ex) {
	...
	private static void internalRefreshStatus(MysterIP ip) {
		...
		//UDP PING SECTION
		try {
			if (UDPPingClient.ping(ip.ip.getIP())) {
				//System.out.println("There has been a successfull PING");
				ip.setStatus(true);
				ip.lastminiupdate=System.currentTimeMillis();
				ip.lastPingTime=(int)(ip.lastminiupdate-time_temp);
				return;
			}
			//ip.setStatus(false);
		} finally {
		...
		try {
			...
			if (System.currentTimeMillis()-ip.lastminiupdate<ip.MINIUPDATETIME) return;	//Don't update if last update was a second ago
						
			MysterSocket s=null;
			try {//If this caused an exception, the ip has no computer attached.
				s=MysterSocketFactory.makeStreamConnection(ip.ip);		
				DataOutputStream out=new DataOutputStream(s.getOutputStream());
				out.writeInt(2);//magic number are fun.
				int whoCares=s.getInputStream().read();
				try {s.close();} catch (Exception ex){}
				ip.setStatus(true);
				ip.lastPingTime=(int)(System.currentTimeMillis()-time_temp);
			} catch (Exception ex) {
				ip.setStatus(false);
				ip.lastPingTime=-2;
				try {s.close();} catch (Exception exp) {}
			}//To keep the value of the last update.
