...
/**
 * The IP list is a list of com.myster objects. The idea behind it is that the data type
 * ie: Tree or linked list of array can be changed without affecting the rest of the program.
 * 
 *
 *
 */
...
class IPList {
	...
	/**
	* Returns a String array of lenght the requested number of entries.
	* Note: It's possible for the list to have fewer entrie than requested..
	* IN that case the rest of the array will be null.
	*
	* This function will not return any items from the list that aren't currently "up"
	* ie: That cannot be connected to because they are down or the user isn't connected
	* to the internet.
	*/
	...
	private synchronized void removeCrap() {
		for (int i=0; i<array.length; i++) {
			for (int j=i+1; j<array.length && array[j]!=null; j++) {
				if (array[i].getAddress().equals(array[j].getAddress())) removeItem(j);
			}
		}
		
		for (int i=0; i<array.length && array[i]!=null;i++) {
			if (array[i].getAddress().getIP().equals("127.0.0.1")) removeItem(i);
		}
	...
	*/	
	private synchronized void save() {
		...
		/**
	* insertionSort adds an IP to the list.. the list currently uses an array and insert
	* into the list using insertion sort. It also checks to make sure the same place
	* isn't put in twice.
	*/
	...
	private synchronized void insertionSort(MysterServer ip) {
		...
		//System.out.println("Asking list "+type+" if it would like ip "+ip.getName());
		//LOOKS FOR A FREE SPACE IN THE LIST AND INSERTS IN THE LIST IF IT FINDS ONE...
		for (int i=0; i<array.length; i++) {
			if (array[i]==null) {
				array[i]=ip;
				//System.out.println("Adding com.myster "+ip.getName()+" to list "+type);
				sort();			//ya gotta sort...
				save();			//Saves the new IP. 
				return;
			}
			if (array[i].getAddress().equals(ip.getAddress())) {
				//System.out.println("List "+type+" already contains "+ip.getName());
				return;
			}
		}
		
		//THIS CODE IS KICK ASS.. IT ONLY SORTS IF THE RANK OF THE LAST ITEM IS LESS THAN IT'S RANK!
		...
		//System.out.println("List "+type+" refused "+ip.getName()+" ranked "+(ip.getRank(type)*100));
	}
