...
/**
	This class is responsible for encapsulating all the crap needed for dealing with 
...
public class MP3Header {
	...
	private final int[][] samplingarray={{44100, 48000, 32000, -1},{22050, 24000,16000}};
	private int bitrate;
	...
	/**
		constructor takes a MP3 file as an argument. If the file is not an MP3 file the routine will return an exception.
	*/
	
	public MP3Header (File f) throws IOException{
		if (!f.exists()) throw new IOException("File does no exist");
		...
		try {
			...
			workingbyte=(byte)(workingbyte>>6); //bit shift right by 6 bits
			workingbyte=(byte)(workingbyte&3);
			
			if (workingbyte==0||workingbyte==1||workingbyte==2) stereo=true;
			else stereo=false;
			...
			workingbyte=(byte)(workingbyte>>3); //bit shift right by 6 bits
			workingbyte=(byte)(workingbyte&1);
			...
			if (head.equals("TAG")) {
				try {
					...
					//System.out.println("Song is "+songName+" by "+artist+" from the album "+album);
				} catch (Exception ex){
					ex.printStackTrace();
				}
			}
	...
	private static String trimMe(String s) {
		...
		else if (index!=-1) s=s.substring(0,index);
		
		for (int i=s.length(); i>1; i--) {
			if(s.charAt(i-1)!=' ') {
				s=s.substring(0, i);
				if (i==1&&s.charAt(0)==' ') s=null;
				break;
			}
		}
		return s;
	...
	*/
	...
	/** returns an int representing the Layer. Encoding is arbitrary so use defined constants. */
	public int getLayer() {
		return layer;
	}
	...
	*/
	...
	public String toString() {
		return "Bitrate: "+bitrate+"bps | Sample Rate: "+samplingrate+"hz | Copyright: "+copyright+" | Stereo: "+stereo+" | "+
