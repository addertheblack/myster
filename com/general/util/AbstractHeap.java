...
abstract public class AbstractHeap extends AbstractCollection implements Heap {
    ...
    public AbstractHeap() { this(4, null); }
    public AbstractHeap(int initialMemoryAllocation) { this(initialMemoryAllocation, null); }
    public AbstractHeap(Comparator comparator) { this(4, comparator); }
	...
	int r = ((i << 1) + 2);
        int found = -1;
        if (l < size_ && isInOrder(data[l], item)) found = find(item, l);
        if (found == -1 && r < size_ && isInOrder(data[r], item)) found = find(item, r);
        return found;
    }
    ...
    public boolean remove(Object item) { 
        ...
        int i;
        try { i = find(item, 0); } 
        catch(ClassCastException e) { i = findKey(item); }
    ...
    public boolean contains(Object item) {
        if (size_ == 0) return false;
        try { return (find(item, 0) != -1); }
        catch(ClassCastException e) { return (findKey(item) != -1); }
    }
	...
	if (l < size_ && isInOrder(data[l], data[i])) higher = l;
	else higher = i;

	if (r < size_ && isInOrder(data[r], data[higher])) higher = r;
    ...
    protected int compare(Object a, Object b) {
        if (comparator == null) return ((Comparable)a).compareTo(b);
        else return comparator.compare(a, b);
    }

    static protected int parent(int i) { return ((i - 1) >> 1 ); }
