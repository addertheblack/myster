...
public class TabPanel extends Panel {
	...
	//This orutine creates a tab with the generic middle graphic and uses the Stirng sent to it as a label.
	public void addTab(String s) {
		addTab(new Tab(s, this));
	}
	
	//THis routine creates a tab with a special graphic (the String) and no label.
	...
	public void paint(Graphics g) {
		...
		g.drawLine(getMaxLength(),getSize().height-1,getSize().width-1,getSize().height-1);
	...
	public void addTab(Tab t) {
		tabs.addTab(t);
		if (tabs.size()==1) tabs.setSelect(0);
		repaint();
	}
	...
	private class TabVector extends Vector {
		...
		public synchronized void setSelect(int tabnum) {
			...
			if (tabnum!=-1) {
				...
				tabs.getTab(lastselected).setSelect(true);//this is dependent on there being no remove tab routine.
				getToolkit().beep();
			}
		...
		public synchronized void paint(Graphics g) {
			...
			int cwxloc=getMaxLength();//current working x location.. for graphics context transpose.
			...
			if (selectedtab!=null) selectedtab.paint(g);
			g.translate(-selectedcwxloc,0);
		}
		...
		public synchronized int getMaxLength() {
			...
			for (int i=0; i<tabs.size(); i++) {
				counter+=getTab(i).getSize().width-overlap; //another good line of code.
			}
		...
		public synchronized int getTabClicked(int x) {
			...
			for (int i=0; i<tabs.size(); i++) {
				counter+=getTab(i).getSize().width-overlap; //another good line of code.
				if (x<counter) return i;
			}
			
			counter+=overlap; //The last one doesn't count;
			if (x<counter) return tabs.size()-1;
			return -1;
		}
	...
	private class ListenerVector extends Vector {
		...
		public synchronized void fireEvents(TabPanel p, int tabid) {
			for (int i=0; i<size(); i++) {
				((TabListener)(elementAt(i))).tabAction(new TabEvent(p, tabid));
			}
		}
	}	
}