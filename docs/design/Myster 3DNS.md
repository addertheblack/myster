...
The public `ThreeDnsLookup.resolve(target)` operation automatically takes one immutable target-specific seed snapshot from `Tracker`. `ThreeDnsLookup` implements the narrow `com.myster.net.client.DnsLookupProtocol` capability exposed by `MysterProtocol`, so feature code can request identity resolution without depending on the traversal implementation. The snapshot is backed by the live pool-wide CID index, includes currently usable address/key candidates, and remains available even when the optional local retained 3DNS list is absent. A successful expected-key encrypted response verifies the responding peer, while every candidate inside that response remains an untrusted hint. If a returned candidate's derived CID matches the target, the resolver queries that candidate next; lookup completes only when the target candidate itself returns an authenticated response. Otherwise, the resolver selects the closest eligible candidate not yet tried and repeats the process.

Application bootstrap constructs stream and datagram capabilities first. `MysterServerPoolImpl`
receives only those capabilities, allowing `Tracker` and then `ThreeDnsLookup` to be constructed
before the immutable `MysterProtocolImpl` aggregate is assembled. This avoids a protocol/tracker
initialization setter while keeping 3DNS discoverable from the protocol stack.
...
TCP has the same underlying identity concept: `TLSSocket.createClientSocket(...)` can compare the peer certificate public key with an expected key. Consumers express that requirement through `ParamBuilder.withExpectedServerPublicKey(...)` and ask `MysterStream` to create the ordinary reusable `MysterSocket`; `MysterStreamImpl` delegates the check to `MysterSocketFactory`. Private-type invitation joining is the first exact-bootstrap consumer: it accepts only `EXACT_VERIFIED`, then reuses the result's full public key as the expected TLS peer while invoking sections 125 and 126 through `MysterStream`. A closest verified peer is routing evidence only and never receives the invitation password.
