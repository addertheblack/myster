...
For each positive-offset target, the worker asks known seeds for `FIND_CLOSEST(target, limit)`. It validates exact and LEFT/predecessor candidates through Part 2a and lets normal pool/listener behavior reconsider validated servers for retention. It does not select RIGHT/successor as the normal next query because that crosses the target in the positive routing direction.
...
| One-target maintenance query | `ThreeDnsMaintenance` | Scheduled and damage-triggered work | Part 2a `findClosest`, candidate validation |
...
Data flows from a scheduled or damage event to one target job, then through one-hop `FIND_CLOSEST` calls and candidate validation, then back into the normal pool refresh listener that updates `ThreeDnsServerList`. Maintenance does not directly write retained entries or preferences.
...
- [ ] Every discovered candidate passes Part 2a validation before normal pool/list retention.
...
5. For a target job, query bounded seeds, process exact then LEFT candidates, validate candidates through Part 2a, and rely on normal pool events to populate retention.
