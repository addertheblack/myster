/**
 * The transaction class is reponsible for encapsulation transaction data /
 * header formating This class does double duty filling in for both to ->
 * transactions and thier replies <- . This leads to code to be more nasty than
 * usual but does very nicely on the code reuse front. To client (replies) and
 * to server (to) packets are formated the same except that to client packets
 * have an extra byte to signal basic protocol level errors. The only error
 * defined at the moment is the TRANSACTION_TYPE_UNKNOWN which is sent by
 * servers who have been asked for data of an unknown type. This itself is here
 * so that unknown transaction type errors can sperated from timeout and other
 * errors that might confuse debugging or lead to ambiguous error messages.
 * <p>
 * This object is intended to be immutable. This object should not be created by
 * outside packages as their are several feilds that need to be filled out that
 * outside code cannot fill out. Since the object is supposed to be immutable it
 * can be sent outside core protocol code after it has been created without fear
 * of corruption. This class should not be subclassed but wrapper instead using
...
public final class Transaction implements DataPacket { //Immutable (Java needs
    ...
    protected Transaction(ImmutableDatagramPacket packet)
            throws NotATransactionException { //makes a Transaction
        ...
        try {
            ...
            if (isForClient()) { //reply packets have a 1 byte longer header.
                                 // THis byte is the error byte. !=0 is err
                int errorTemp = in.read();
                if (errorTemp == -1)
                    throw new NotATransactionException(
                            "Transaction shorter than header.");
                errorByte = (byte) errorTemp;
            } else {
                errorByte = 0;
            }
        } catch (IOException ex) {
            throw new NotATransactionException("Formating error occured: "
                    + bytes.length + " " + packet.getAddress() + " " + ex);
        }
    ...
    public Transaction(Transaction transaction, byte[] bytes, byte errorByte) {
        this(transaction.getAddress(), transaction.getTransactionCode(),
                transaction.getConnectionNumber(), bytes, !transaction
                        .isForClient(), errorByte);
    }
    ...
    public byte[] getHeader() { //slow
        ...
        try {
            ...
            ex.printStackTrace(); //!!!!!! should never happen since all calls
                                  // are to byte[]
        }
    ...
    private int getHeaderSize() { //sizeToSkip for header! (should be equal to
                                  // getBytes().length)
        return HEADER_SIZE + (isForClient() ? 1 : 0); //reply packets are 1
                                                      // byte longer (the err
                                                      // packet)
    }
